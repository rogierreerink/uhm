...
pub async fn handle(Path(id): Path<Uuid>, State(state): State<Arc<AppState>>) -> impl IntoResponse {
    ...
    let unit = match db_conn.query_one(&stmt, &[&id]).await {
        Ok(row) => Unit {
            id: row.get("id"),
            name: Plural {
        ...
        Err(err) => {
            tracing::warn!("Unit could not be found: {}", err);
