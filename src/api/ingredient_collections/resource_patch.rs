use crate::db::ingredient_collections::{query, upsert};
...
pub async fn handle(
    ...
    let _ = match query::query_one(&transaction, &id).await {
        Ok(item) => item,
    ...
    if let Err(err) = upsert::upsert(&transaction, &upsert::Resource { id }).await {
