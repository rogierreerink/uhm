...
<style>
	button {
		...
		align-items: center;
		padding: 0.3em 0.4em;
		border: 1px solid var(--button-border-color, var(--color-zinc-700));
		border-radius: 0.25em;
		box-shadow: 0.25em 0.25em 0 var(--button-shadow-color, var(--color-zinc-950));
		background-color: var(--button-bg-color, var(--color-zinc-800));
		color: var(--button-color, var(--color-zinc-400));
		cursor: pointer;
		user-select: none;
	...
	button:active {
		...
		box-shadow: 0.125em 0.125em 0 var(--button-shadow-color, var(--color-zinc-950));
	}
